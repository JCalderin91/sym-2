...
$(document).ready(function () {
    ...
    if(validate()){
      ...
      do {
        ...
        if (lastEvent == 'E1') {
          ...
          let newTr = parseFloat(results[results.length - 1].le[0].t) + parseFloat(t)
      ...
      sortArrayEvent(inPatter, 'asc').forEach((item, key) => {
        template += `<tr>
                      <td>Exp${item.cant+1} = -82.5988 m/c Ln ${item.pseudo} = ${item.t} seg</td>
                    </tr>`;
      })
      ...
      sortArrayEvent(servicePatter, 'asc').forEach((item, key) => {
        template += `<tr>
                      <td>Weilbull${item.cant+1} = 2.01403+ 98.2327*(-ln(${item.pseudo})))^(1/(1.28966) = ${item.t} seg</td>
                    </tr>`;
      })
  ...
  function verify__results(results, inPatter, servicePatter) {
    ...
    response.push({
      'title': 'Tiempo promedio entre llegadas (seg)',
      'value': aux
    })
    ...
    response.push({
      'title': 'Tiempo promedio de servicios (seg)',
      'value': aux
    })
    ...
    response.push({
      'title': 'Tiempo promedio de espera en el sistema (seg)',
      'value': total
    })
    ...
    if (isNaN(total)) total = 0
    response.push({
      'title': 'Tiempo promedio de espera en cola (seg)',
      'value': total
    })
